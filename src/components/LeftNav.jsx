...
const LeftNav = () => {
  return (
    <div className='fixed top-0 left-0 h-screen lg:w-[224px] w-[60px] flex flex-col justify-between flex-shrink-0 bg-[#1E2640] text-white py-[16px] px-[10px]'>
      <div>
        <div className='hidden lg:flex w-full items-center justify-around pb-[24px] '>
        ...
        {
          leftNavTabs.map(tab => (
            <div className={`cursor-pointer rounded-[4px] py-[12px] lg:px-[16px] px-[6px] w-full flex text-white  ${tab.tabName === "Payments" ? "rounded-[4px] bg-white bg-opacity-10" : " opacity-[0.8]"} hover:rounded-[4px] hover:bg-white hover:bg-opacity-10`} key={tab.tabName}>
              <img src={`./images/leftNavImages/${tab.tabImage}.svg`} className='w-[20px] h-[20px] ml-[5px] mr-[12px]' />
              <div className="text-[14px] hidden lg:block text-[#fff] font-medium">
                {tab.tabName}
              </div>
            </div>
